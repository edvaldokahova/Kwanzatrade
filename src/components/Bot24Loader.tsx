...
export default function Bot24Loader({ show }: { show: boolean }) {
  // 1. BLOQUEIO DE ACESSO: Garante que o utilizador não consiga clicar nem fazer scroll
  useEffect(() => {
    if (show) {
      document.body.style.overflow = "hidden"; // Trava o scroll
      document.body.style.pointerEvents = "none"; // Desativa cliques no site (lá no fundo)
    } else {
      document.body.style.overflow = "unset";
      document.body.style.pointerEvents = "auto"; // Reativa tudo
    }
    return () => {
      document.body.style.overflow = "unset";
  ...
  return (
    /* 2. EFEITO EMBACEADO E FUNDO: O container que cobre tudo */
    <div className="fixed inset-0 z-[99999] flex flex-col items-center justify-center bg-[#0b0b0c]/70 backdrop-blur-xl pointer-events-auto">
      
      <div className="relative">
        {/* O TEU SVG MARAVILHOSO */}
        <svg width="220" height="220" viewBox="0 0 300 300" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M40 100 L150 150 L260 100" stroke="#444" strokeWidth="2" strokeDasharray="8 8"/>
          
          <path d="M40 200 L150 150" stroke="white" strokeWidth="6" strokeLinecap="round" opacity="0.9"/>
          
          <circle className="core-glow" cx="150" cy="150" r="12" fill="#00F0FF" style={{ filter: "blur(10px)" }}/>
